# Prismax Backend 测试框架使用指南 🚀

## 📋 目录

1. [环境配置](#环境配置)
2. [虚拟环境使用](#虚拟环境使用)
3. [运行测试](#运行测试)
4. [测试报告](#测试报告)
5. [调试技巧](#调试技巧)
6. [常见问题](#常见问题)

---

## 环境配置

### ✅ 已验证的功能

1. **用户管理服务** - https://user.prismaxserver.com
   - ✅ 健康检查接口正常
   - ✅ HTTP请求封装工作正常
   - ✅ 日志记录完整

2. **机器人控制服务** - https://teleop.prismaxserver.com/
   - ✅ 机器人状态接口正常
   - ✅ 返回实时机器人数据 (arm1: 220人队列, arm2: 可用)
   - ✅ 数据验证通过

### 🛠️ 环境配置

**当前环境**: Beta
- 用户管理: https://user.prismaxserver.com
- 机器人控制: https://teleop.prismaxserver.com/

**切换环境**:
```bash
# 使用本地环境
export TEST_ENV=local

# 使用生产环境
export TEST_ENV=live
```

### 📊 测试环境状态

```
Python: 3.12.8 (testenv)
pytest: 7.4.3
依赖包: 29个已安装
测试状态: ✅ 正常运行
```

---

## 虚拟环境使用

### 🚀 三种使用方式

#### 方式1: 手动激活（推荐）

```bash
cd /Users/wanxin/PycharmProjects/Knowlege_PrismaX/QA_PrismaX/backend-test/Live_Framework

# 激活虚拟环境
source testenv/bin/activate

# 现在可以运行测试
pytest -v

# 完成后退出
deactivate
```

#### 方式2: 使用激活脚本

```bash
cd /Users/wanxin/PycharmProjects/Knowlege_PrismaX/QA_PrismaX/backend-test/Live_Framework

# 一键激活
./activate.sh
```

#### 方式3: 一行命令运行测试

```bash
cd /Users/wanxin/PycharmProjects/Knowlege_PrismaX/QA_PrismaX/backend-test/Live_Framework
source testenv/bin/activate && pytest -v
```

### 💡 如何确认在虚拟环境中？

**方法1: 查看命令提示符**
```bash
# 激活前
user@host:~/Live_Framework$

# 激活后（注意前面的 (testenv)）
(testenv) user@host:~/Live_Framework$
```

**方法2: 查看Python路径**
```bash
which python
# 在虚拟环境中: /Users/wanxin/.../testenv/bin/python
# 不在虚拟环境: /usr/local/bin/python3
```

**方法3: 检查环境变量**
```bash
echo $VIRTUAL_ENV
# 在虚拟环境中: /Users/wanxin/.../testenv
# 不在虚拟环境: (空)
```

### 🔧 虚拟环境管理

**查看已安装的包**:
```bash
source testenv/bin/activate
pip list
```

**安装新包**:
```bash
source testenv/bin/activate
pip install package-name
pip freeze > requirements.txt  # 更新依赖列表
```

**重建虚拟环境**:
```bash
# 删除旧环境
rm -rf testenv

# 创建新环境
python3 -m venv testenv

# 激活并安装依赖
source testenv/bin/activate
pip install -r requirements.txt
```

---

## 运行测试

### 📝 日常工作流程

**开始测试**:
```bash
# 1. 进入目录
cd /Users/wanxin/PycharmProjects/Knowlege_PrismaX/QA_PrismaX/backend-test/Live_Framework

# 2. 激活虚拟环境
source testenv/bin/activate

# 3. 确认环境（可选）
which python    # 应显示 .../testenv/bin/python
pytest --version

# 4. 运行测试
pytest -v
```

**结束测试**:
```bash
# 退出虚拟环境
deactivate
```

### 🚀 运行测试的多种方式

#### 方式1: 运行单个测试

```bash
# 测试用户管理 - 健康检查
pytest test_cases/test_user_management.py::TestHealthCheck::test_health_check -v

# 测试机器人状态
pytest test_cases/test_tele_op.py::TestRobotStatus::test_get_robots_status -v

# 测试用户管理 - 所有健康检查
pytest test_cases/test_user_management.py::TestHealthCheck -v

# 测试机器人 - 所有状态检查
pytest test_cases/test_tele_op.py::TestRobotStatus -v
```

#### 方式2: 运行所有测试

```bash
# 运行所有用户管理测试
pytest test_cases/test_user_management.py -v

# 运行所有机器人控制测试
pytest test_cases/test_tele_op.py -v

# 运行所有测试
pytest -v
```

#### 方式3: 使用便捷脚本

```bash
# 运行所有测试
./run_tests.sh

# 运行用户管理测试
./run_tests.sh -s user

# 运行机器人控制测试
./run_tests.sh -s tele

# 运行并生成报告
./run_tests.sh -r

# 并行运行测试
./run_tests.sh -p 4

# 清理后运行
./run_tests.sh --clean -r
```

### 🎯 常用测试场景

**1. 快速验证服务可用性**:
```bash
# 只运行健康检查
pytest test_cases/test_user_management.py::TestHealthCheck -v
pytest test_cases/test_tele_op.py::TestRobotStatus::test_get_robots_status -v
```

**2. 测试特定功能**:
```bash
# 测试用户认证
pytest test_cases/test_user_management.py::TestAuthentication -v

# 测试队列管理
pytest test_cases/test_tele_op.py::TestQueueManagement -v
```

**3. 运行所有未跳过的测试**:
```bash
# 运行所有没有@pytest.mark.skip标记的测试
pytest -v
```

**4. 显示详细输出**:
```bash
# 显示所有print和日志
pytest test_cases/test_user_management.py::TestHealthCheck -v -s
```

### 📊 测试结果示例

**用户管理服务 - 健康检查**:
```
✅ PASSED - test_health_check
响应: 200 OK
状态: "ok"
耗时: 0.26s
```

**机器人控制服务 - 状态查询**:
```
✅ PASSED - test_get_robots_status  
响应: 200 OK
机器人数据:
  - arm1: 队列220人, 不可用, 直播中
  - arm2: 队列0人, 可用, 已暂停
耗时: 0.66s
```

---

## 测试报告

### 📈 生成Allure报告（推荐）

```bash
# 方式1: 运行测试并自动打开报告
./run_tests.sh -r

# 方式2: 手动生成
pytest
allure serve test_report/allure-results
```

### 📄 生成HTML报告

```bash
# 生成HTML报告
pytest --html=test_report/report.html --self-contained-html
```

### 📁 查看测试日志

```bash
# 查看今天的测试日志
cat logs/test_$(date +%Y%m%d).log

# 实时查看日志
tail -f logs/test_$(date +%Y%m%d).log

# 查看最近10条日志
tail -n 10 logs/test_$(date +%Y%m%d).log

# 查看详细的HTTP请求
cat logs/test_$(date +%Y%m%d).log | grep "Request:"
```

---

## 调试技巧

### 🔍 调试方法

**1. 运行失败的测试**:
```bash
# 只运行上次失败的测试
pytest --lf -v

# 先运行失败的，再运行其他的
pytest --ff -v
```

**2. 进入调试模式**:
```bash
# 失败时自动进入pdb调试器
pytest --pdb
```

**3. 查看详细的HTTP请求**:
```bash
# 所有日志都在 logs/test_YYYYMMDD.log 中
cat logs/test_$(date +%Y%m%d).log | grep "Request:"
```

**4. 显示执行时间**:
```bash
# 显示最慢的10个测试
pytest --durations=10

# 显示所有测试的执行时间
pytest --durations=0
```

---

## 常见问题

### Q1: 忘记激活虚拟环境？

**问题**: 
```bash
# 看到这个错误：
pytest: command not found
```

**解决方法**:
```bash
source testenv/bin/activate
```

### Q2: 虚拟环境损坏？

**解决方法**:
```bash
# 重建虚拟环境
rm -rf testenv
python3 -m venv testenv
source testenv/bin/activate
pip install -r requirements.txt
```

### Q3: 如何共享环境给团队？

```bash
# 导出依赖
pip freeze > requirements.txt

# 团队成员创建自己的环境
python3 -m venv testenv
source testenv/bin/activate
pip install -r requirements.txt
```

### Q4: 测试失败如何查看详细信息？

```bash
# 方法1: 查看Allure报告
allure serve test_report/allure-results

# 方法2: 查看日志文件
cat logs/test_$(date +%Y%m%d).log

# 方法3: 重新运行失败的测试并显示详细输出
pytest --lf -v -s
```

---

## 📋 测试覆盖范围

### 用户管理服务 (28个接口)
- [x] `/health` - 健康检查
- [x] `/api/get-users` - 获取用户信息
- [ ] `/auth/send-verification-code` - 发送验证码 (需要真实邮箱)
- [ ] `/auth/verify-code` - 验证验证码 (需要真实验证码)
- [ ] 其他接口... (参见 test_user_management.py)

### 机器人控制服务 (14个接口)
- [x] `/robots/status` - 机器人状态
- [x] `/get_live_paused_status` - 直播暂停状态  
- [ ] `/queue/join` - 加入队列 (需要认证)
- [ ] `/queue/status` - 队列状态 (需要认证)
- [ ] 其他接口... (参见 test_tele_op.py)

---

## 🎨 IDE配置

### PyCharm
1. 打开项目
2. Preferences → Project → Python Interpreter
3. 添加解释器 → Existing Environment
4. 选择: `testenv/bin/python`

### VS Code
1. Cmd+Shift+P
2. "Python: Select Interpreter"
3. 选择 `testenv/bin/python`

---

## ⚠️ 注意事项

### ✅ 要做的
- ✅ 始终在虚拟环境中运行测试
- ✅ 定期更新 `requirements.txt`
- ✅ 与团队分享 `requirements.txt`，而不是虚拟环境目录
- ✅ 测试前清理旧的缓存和日志

### ❌ 不要做的
- ❌ 不要提交 `testenv/` 到Git（已在.gitignore中）
- ❌ 不要在系统Python中安装项目依赖
- ❌ 不要直接复制虚拟环境给他人

---

## 📚 相关文档

- **README.md** - 完整使用文档
- **QUICK_START.md** - 快速开始指南
- **TEST_DESIGN.md** - 测试设计文档
- **INSTALL.md** - 安装指南

---

## ✨ 下一步

1. **熟悉虚拟环境操作**
   ```bash
   source testenv/bin/activate
   pytest --version
   deactivate
   ```

2. **运行第一个测试**
   ```bash
   source testenv/bin/activate
   pytest test_cases/test_user_management.py::TestHealthCheck -v
   ```

3. **添加更多测试用例**
   - 移除更多 @pytest.mark.skip 标记
   - 准备真实的测试账号和数据

4. **配置CI/CD**
   - 集成到GitLab CI / GitHub Actions
   - 自动运行测试并生成报告

5. **扩展测试覆盖**
   - 添加性能测试
   - 添加压力测试
   - 添加安全测试

---

## 💡 提示

- 测试日志保存在 `logs/` 目录
- Allure报告保存在 `test_report/` 目录
- 所有接口的详细请求/响应都会被记录
- 虚拟环境目录 `testenv/` 已被 `.gitignore` 忽略

---

**测试框架状态**: ✅ 正常运行  
**虚拟环境**: ✅ 已创建并验证  
**Python**: 3.12.8 (testenv)  
**pytest**: 7.4.3  
**依赖**: 29个包已安装  
**最后验证时间**: 2025-10-06  
**维护者**: QA Team

